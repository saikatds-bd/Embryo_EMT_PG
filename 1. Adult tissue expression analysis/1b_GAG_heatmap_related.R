...
genes_1 <- c(genes, emt_tf)

data.subset <- data %>% dplyr::filter(`Gene name` %in% genes_1)
...
data.subset <- data.subset %>% select(c("Gene name", "Cell type", "Expression"))
...
pdf("Ward_GAG_HPA_Stability_CellType.pdf", height = 3, width = 4.5)
...
pdf("Ward_GAG_HPA_Stability_Genes.pdf", height = 3, width = 4.5)
#internal <- clValid::clValid(t(expr), method = "complete", metric = "correlation", clMethods = "hierarchical", nClust = 2:10, validation = "internal")
...
writexl::write_xlsx(expr_df, "GAGs_in_adult_tissue.xlsx")
...
subgroup_cols <- c("Initiation" = "sienna1", "Polymerization" = "#1D4E89", "Sulfation" = "#56B4E9", 
                   "Epimerization" ="blue", 
                   "Regulation" = "#F4A261", "Modification" = "red")

ann3$Module <- str_to_upper(ann3$Module)
ann3$Module <- gsub("LINKER", "Linker", ann3$Module)
row_ha <- rowAnnotation(
  "Module" = ann3$Module,
  "Subgroup" = ann3$Subgroup,
  ...
  annotation_legend_param = list(
    Module = list(title = "Biosynthetic Groups"),
    Subgroup = list(title = "Enzymatic Functions")
  )
)
set.seed(123)
...
ht <- Heatmap(h, col = col_fun,
              ...
              heatmap_legend_param = list(title="Log2+1 Expression", legend_height=unit(3, "cm"),title_gp=gpar(fontsize=9, fontface="bold"),labels_gp = gpar(fontsize=8)),
              row_split = 5, 
              column_split = 4#,
              #column_gap = unit(c(1,1,1,1,1), "mm"),
              #row_gap = unit(c(1,1,1,1), "mm")
              )



draw(ht,column_title = "GAG enzymes in Normal Tissue Cell Types", column_title_gp = gpar(fontsize = 8), 
...
pdf("WARD_HPA_GAG_Saikat.pdf",  width=20,height=18)
print(ht)
dev.off()
...
write.csv2(ann3_merged, file = "WARD_HPA_GAG_Gene_Order.csv")


############################
# cluster col and row info #
############################
...
ann2_merged <- ann2_merged[,c(4,2,5)]

write.csv2(ann2_merged, "WARD_HPA_GAG_CellType_Order.csv")
...
names(merged_ann) <- c("Cell Type", "origin", "Cluster", "gene", "expression")

merged_ann <- merged_ann %>% mutate(Cluster = factor(Cluster))

# 1) Compute all pairwise exprs per gene, FDR-adjusted
pw <- merged_ann %>%
  group_by(gene) %>%
  pairwise_t_expr(expression ~ Cluster, p.adjust.method = "fdr") %>%
...
pdf("Ward_HPA_GAG_EMT_Genes_Saikat", height = 8, width = 6)
print(p)
dev.off()
...
cell_table
s <- as.data.frame(table(cell_table$origin, cell_table$Cluster, useNA = "ifany"))
...
# Perform Chi-Square expr
fish_result <- fisher.expr(fish)
print(fish_result)
...
tileplot <- tileplot + 
  ggtitle(paste("Fisher's Exact expr p-value:", p_value)) +
  theme(plot.title = element_text(size = 12, hjust = 0.5))

#tileplot + theme(text = element_text(size = 9))

pdf("Fisher expr_Ward_HPA_GAG.pdf",
    width = 6, height = 5)
print(tileplot)
dev.off()
