...
## --- Build lineage-specific count matrices  ------------------
...
goi <- intersect(unique(c(emt_tfs, proteoglycans$Gene, gag$Gene)), rownames(counts_meso))
genes <- goi
...
association_dataframe <- list("Endoderm Association" = asso_endo,
                      "Mesoderm Association" = asso_meso)

writexl::write_xlsx(association_dataframe, "Association_towards_Mesoderm.xlsx")

save(sce_endo, cds_endo, sce_meso, cds_meso, asso_meso, asso_endo, counts_endo, counts_meso,
     proteoglycans, gag, emt_tfs, file = "Embryo_Separate_CDS.RData")
